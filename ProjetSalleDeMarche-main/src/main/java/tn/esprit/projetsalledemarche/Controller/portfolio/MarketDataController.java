package tn.esprit.projetsalledemarche.Controller.portfolio;
...
import tn.esprit.projetsalledemarche.Entity.Linda.portfolio.*;
...
private final UserRepository userRepository;
    ...
    public ResponseEntity<List<MarketData>> getChartData(@RequestParam String interval) {
        List<MarketData> data = marketDataService.getDataByInterval(interval);
    ...
    public ResponseEntity<Void> sendIndicators(@PathVariable String symbol, @RequestBody IndicatorsDTO indicators) {
        // Enregistrer l'indicateur avec le symbole
        indicatorService.saveIndicator(symbol, indicators);
        return ResponseEntity.ok().build(); // Retourne 200 OK
    }

    // Méthode pour récupérer les indicateurs
    @GetMapping("/get-indicators/{symbol}")
    public ResponseEntity<List<IndicatorsDTO>> getIndicators(@PathVariable String symbol) {
        List<IndicatorsDTO> indicators = indicatorService.getIndicators(symbol);

        if (indicators != null && !indicators.isEmpty()) {
            return ResponseEntity.ok(indicators); // Retourne la liste des indicateurs
        } else {
            return ResponseEntity.notFound().build(); // Retourne 404 si aucun indicateur trouvé
        }
    }
    
    @PutMapping("/update-balance/{portfolioId}")
    public ResponseEntity<Double> updateBalance(@PathVariable Long portfolioId, @RequestBody Double profitOrLoss) {
        Portfolio portfolio = portfolioRepository.findById(portfolioId)
                .orElseThrow(() -> new RuntimeException("Portfolio not found"));

        // Mettre à jour le solde du portefeuille
        portfolio.setBalence(portfolio.getBalence() + profitOrLoss);

        // Sauvegarder le portefeuille mis à jour
        portfolioRepository.save(portfolio);

        // Retourner le nouveau solde
        return new ResponseEntity<>(portfolio.getBalence(), HttpStatus.OK);
    }
    ...
    @GetMapping("/get-balence/{id}")
    public ResponseEntity<Double> getBalance(@PathVariable Long id) {
        // Trouver le portefeuille par son identifiant
        Portfolio portfolio = portfolioRepository.findById(id)
                .orElseThrow(() -> new RuntimeException("Portfolio not found for id: " + id));

        System.out.println("Portfolio found for id: " + id);  // Log pour vérifier le portefeuille trouvé

        // Récupérer le solde du portefeuille
        double balance = portfolio.getBalence();
